...
type ExpenseRow = {
  ...
  pendingSync: boolean; // NEW — true for offline-queued expenses not yet saved to the server
};
    ...
    () => [
        ...
        cellRenderer: (params: { data: ExpenseRow }) => {
          ...
          return (
            <div style={{ display: "flex", alignItems: "center", gap: 6, height: "100%" }}>
              <span style={{ opacity: row.pendingSync ? 0.6 : 1 }}>{row.description}</span>
              {row.isRecurring && (
      ...
      },
      {
        ...
        cellRenderer: (params: { data: ExpenseRow }) => {
          ...
          // NEW — check this FIRST, before ownership/permission logic runs at
          // all. A queued-offline expense has no real ID yet, isn't owned by
          // anyone server-side, and can't be edited/deleted/requested until
          // it actually syncs — so none of the normal action buttons apply.
          if (row.pendingSync) {
            return (
